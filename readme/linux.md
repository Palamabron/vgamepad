...
- Virtual devices spoof `input_id` (vendor/product) matching real hardware so SDL2 uses `SDL_GameControllerDB` for correct button mapping. This avoids the "button order mess" that previously broke `BTN_MODE` (Guide).
...
``register_notification()`` reads FF events on the **same** kernel file descriptor libevdev uses for the uinput device. Re-opening ``/dev/input/event*`` will block waiting for events that never arrive.

The implementation opens ``/dev/uinput`` (or ``/dev/input/uinput``) and passes that fd into ``create_uinput_device(uinput_fd=...)``. Without that, libevdev uses managed mode and rumble callbacks stay disabled.
